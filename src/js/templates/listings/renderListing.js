...
const createCardBody = (title, description, deadline, company, created, requirements, tags) => {
  ...
  const editDeleteBtnSection = editDeleteBtnContainer();
  element.append(
    h1,
    detailsContainer,
    btnContainer,
...
const createBtnContainer = () => {
  ...
  const favIcon = createElement(
    ...
    null,
    null,
    '/public/assets/icons/heart-fav.svg'
  );
...
const editDeleteBtnContainer = () => {
  if (localStorage.getItem('role') === 'Client') {
    ...
    const editBtn = createElement(
      'button',
      ['btn', 'btn-theme-secondary', 'text-uppercase', 'px-5', 'me-4'],
      null,
      'edit'
    );
    ...
    const deleteBtn = createElement(
      'button',
      ['btn', 'btn-theme-dark', 'text-theme-light', 'text-uppercase', 'px-4', 'me-4'],
      // null,
      { className: 'greeting' },
      'delete'
    );

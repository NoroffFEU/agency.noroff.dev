...
export function createListing() {
  ...
  button.addEventListener('click', (event) => {
    ...
    if (results.title.length >= 3 && results.location.length >= 3 && deadline > currentTime) {
      ...
      modalText = `<img src="/assets/icons/checkmark.svg" alt="" data-bs-dismiss="modal" aria-label="Close"/>
      <h3>Success</h3>`;
      return (modalBody.innerHTML = `${modalText}`);
    } else {
      modalText = `<img src="/assets/icons/cancel.svg" alt="" data-bs-dismiss="modal" aria-label="Close"/>

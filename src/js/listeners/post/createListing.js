...
export function createListing() {
  ...
  button.addEventListener('click', (event) => {
    ...
    if (results.title.length >= 3 && results.location.length >= 3 && deadline > currentTime) {
      ...
      modalText = `<img src="/public/assets/icons/checkmark.svg" alt="Checkmark icon" data-bs-dismiss="modal" aria-label="Close"/>
      <h3>Success</h3>`;
      return (modalBody.innerHTML = `${modalText}`);
    } else {
      modalText = `<img src="/public/assets/icons/cancel.svg" alt="Cancel icon" data-bs-dismiss="modal" aria-label="Close"/>
